...
use super::{
    ...
    write_to_reg_and_set_flags, FLAG_REG_IDX
};

const OP_CODE: &'static str = "add";

pub fn execute_add(sim86: &mut Simulator8086, instruction: &instruction) -> Transition {
...
fn reg_to_reg(sim86: &mut Simulator8086, op_one: &mut instruction_operand, op_two: &mut instruction_operand) -> Transition {
    let src_value = Wrapping(read_from_reg(sim86, op_two));
